...
J = sum(1/m*sum(-y_vec.*log(h)-(ones(m,num_labels)-y_vec).*log(ones(m,num_labels)-h)));
...
[D2_rn,D2_cn                                       yy] = size(Delta2);
[D1_rn,D1_cn] = size(Delta1);
Theta2_grad = Delta2/m+[zeros(D2_rn,1),(lambda/m)*Theta2(:,2:end)];  
Theta1_grad = Delta1/m+[zeros(D1_rn,1),(lambda/m)*Theta1(:,2:end)];
% =========================================================================
% for t = 1:m
%     a_1 =[ones(m,1),X];
...
% 
% Theta2_grad(:,2:end) = 1/m * D2(:,2:end) + lambda * Theta1(:,2:end);
